...
def encrypt_enigma(text, rotor_seq, rotor_ring, rotor_windows, reflector_id, plugboard_settings):
    ...
    for char in text:
        ...
        for i in range(2, len(rotor_seq) + 1):
            if rotor_windows[-1 * (i - 1)] == rotor_notches[-1 * (i - 1)]:
        ...
        for i in range(1, len(rotor_seq) + 1):
            ...
            propagate_char = int_to_char((char_to_int(propagate_char) + advance_number) % 26)

        print(propagate_char)
        propagate_char = reflector_mappings[reflector_id][propagate_char]
        print("to", propagate_char)

        propagate_char = int_to_char((char_to_int(propagate_char) + char_to_int(rotor_windows[0])) % 26)
...
def main():
    ...
    encrypted_text = encrypt_enigma(input_string2, rotor_sequence, rotor_ring_key, rotor_windows, reflector_id, plugboard_settings)
